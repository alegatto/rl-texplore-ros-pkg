/** \file PolicyGradientEnv.hh
   Definisce l'ambiente per la camminata a frame del RobovieX
    \author agatto
*/

#ifndef _POLICYGRADIENTENV_H_
#define _POLICYGRADIENTENV_H_
...
class PolicyGradientEnv: public Environment {
public:

  /** Creates il dominio per lo sviluppo della camminata del RobovieX.
   * INPUT : rand - generatore numeri aleatori
  */
  PolicyGradientEnv(Random &rand);

  /** Creates il dominio per lo sviluppo della camminata del RobovieX.
    * INPUT : rand - generatore numeri aleatori
    *         isReale - booleano per selezionare tra robot reale e simulato
    *         numTest - intero che definisce il numero di perturbazioni aleatorie per ogni iterazione
  */
  PolicyGradientEnv(Random &rand, bool isReale, int numTest, int numRep, float reward_obbiettivo);
  

  virtual ~PolicyGradientEnv();
...
//--------------------------------------------------------------------------------
  ...
  virtual void getLearningRate(std::vector<float> *learnRate);
  virtual void getEpsilonParam(std::vector<float> *epsilonDif);
  virtual float getNormaVector(std::vector<float> policyVector);
...
protected:
  ...
  double fstable;
     ...
     //Definisce il numero di perturbazioni
     int numRandomPert;
     
     //Definisce il numero di volte che una stessa azione viene valutata
     int numRepetitions;
     
     //Definisce il valore soglia da raggiungere con la reward
     float rewardObiettivo;
